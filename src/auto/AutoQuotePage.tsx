...
export default function AutoQuotePage() {
  ...
  return <div className="page-shell auto-page">
    ...
    <main className="quote-main">
      <div className="auto-demo" role="note"><strong>Entorno de prueba</strong><span>Cotizaciones del sandbox de Galeno. No se emiten pólizas ni se envían emails.</span></div>
      ...
      <div className="quote-card">
        ...
        <section className="form-step auto-form">
          <div className="section-heading"><p className="eyebrow">SEGURO DE AUTO · GALENO SEGUROS</p><h1 ref={heading} tabIndex={-1}>{step === 1 ? "Tu próximo camino, más tranquilo" : step === 2 ? "Una cobertura a tu medida" : "Tu cotización de auto"}</h1><p>{step === 1 ? "Contanos qué auto tenés y encontrá una cobertura para acompañarte." : step === 2 ? "Completá los datos de tu auto y dónde lo guardás." : "Estas son las opciones devueltas por Galeno en su entorno de prueba."}</p></div>
          ...
          {step === 3 && quote && <>
            ...
            {!quote.coverages.length && <p className="auto-pending" role="status">Galeno no devolvió coberturas disponibles para estos datos. Podés corregirlos e intentar nuevamente.</p>}
            <div className="auto-results">{quote.coverages.map(item => <article key={item.code}><span className="auto-result-tag">Galeno · Sandbox</span><ShieldCheck size={28} /><h2>{item.name}</h2><div className="auto-price">{money(item.firstInstallment)}</div><small>Primera cuota</small>{item.remainingInstallment > 0 && <small>Cuotas restantes: {money(item.remainingInstallment)} cada una</small>}<small>Premio informado: {money(item.premium)}</small>{item.deductible && <p>Franquicia: {item.deductible}</p>}<details><summary>Ver detalle de cobertura</summary><ul>{item.benefits.map((benefit, index) => <li key={index}>{benefit}</li>)}</ul></details></article>)}</div>
            ...
            <p className="auto-pending">Resultado de prueba, sin validez para contratar. Referencia: {quote.requestId || "No informada"}.</p>
