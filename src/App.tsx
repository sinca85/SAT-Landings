...
function QuoteStep({ form, quote, onBack, onContract }: { form: FormState; quote: HomeQuote; onBack: () => void; onContract: () => void }) {
  ...
  return <section className="form-step quote-step" aria-labelledby="quote-title">
    ...
    </div><div className="quote-footer"><button className="text-button" type="button" onClick={onBack}><ArrowLeft size={17} /> Corregir mis datos</button></div>
  </section>;
}
  ...
  return <section className="form-step contract-step" aria-labelledby="contract-title">
    ...
    <form onSubmit={onSubmit}>
      <div className="contract-fields">
        ...
        <div className="field"><label htmlFor="contract-apartment">Departamento <span className="optional">(opcional)</span></label><input id="contract-apartment" maxLength={4} placeholder="Ej: B" value={data.apartment} onChange={event => setData(current => ({ ...current, apartment: event.target.value.slice(0, 4) }))} /></div></>}
...
function ContractSuccess({ form, contract, quote, whatsappUrl }: { form: FormState; contract: ContractState; quote: HomeQuote; whatsappUrl: string }) {
  return <section className="form-step contract-success"><span className="success-icon large"><Check size={36} strokeWidth={3} /></span><h1>¡Recibimos tu solicitud, {contract.firstName}!</h1><p>Ya estamos preparando tu propuesta de <strong>Seguro de Hogar Allianz.</strong></p><div className="success-quote"><div><small>Tu cotización</small><img src="/assets/allianz.png" alt="Allianz" /></div><div><strong>{form.homeType} · {quote.quotedSquareMeters} m²</strong><span>Código postal {form.postalCode}</span><b>{formatCurrency(quote.monthlyPrice)} <em>/ mes</em></b><span>12 cuotas fijas · Póliza anual</span><small className="quote-disclaimer">*Precio de referencia, sujeto a ubicación exacta de tu hogar.</small></div></div><section className="next-steps"><h2>¿Qué pasa ahora?</h2><div className="next-steps-grid"><div><span><Check size={21} /></span><strong>1. Solicitud recibida</strong><p>Ya tenemos tus datos y la cotización.</p></div><ArrowRight className="next-arrow" /><div><span><ShieldCheck size={21} /></span><strong>2. Preparamos tu propuesta</strong><p>Revisamos la información para preparar la emisión.</p></div><ArrowRight className="next-arrow" /><div><span><Phone size={21} /></span><strong>3. Te contactamos en el día</strong><p>Un asesor se va a comunicar para revisar la propuesta con vos.</p></div><ArrowRight className="next-arrow" /><div><span><CreditCard size={21} /></span><strong>4. Elegís el medio de pago y emitimos</strong><p>Una vez confirmada, solicitaremos el medio de pago que prefieras para finalizar.</p></div></div><div className="no-payment"><ShieldCheck size={28} /><div><strong>No tenés que hacer nada ahora.</strong><span>Todavía no realizaste ningún pago y tu póliza aún no fue emitida.</span></div></div></section><a href={whatsappUrl} target="_blank" rel="noreferrer" className="success-whatsapp"><MessageCircle size={34} /><span><strong>¿Querés hablar con nosotros ahora?</strong><small>Escribinos por WhatsApp y te ayudamos.</small></span><b>Hablar por WhatsApp <ArrowRight size={18} /></b></a><div className="success-commitments"><span><CircleDollarSign size={25} /><b>Nuestro compromiso</b><small>Cotizamos y emitimos en el día.</small></span><span><UserRound size={25} /><b>Asesoramiento humano</b><small>Te acompañamos en todo el proceso.</small></span><span><ShieldCheck size={25} /><b>Tu tranquilidad, nuestra prioridad</b><small>Trabajamos con Allianz, una de las compañías de seguros líderes del mundo.</small></span></div></section>;
}
...
function HomeQuotePage() {
  ...
  useEffect(() => { void fetch(`${API_URL}/api/config`).then(response => response.ok ? response.json() : null).then(data => { const entry = data?.entries?.find((item: { type?: string }) => item.type === "whatsapp"); const number = entry?.value?.replace(/\D/g, ""); if (number) setWhatsappUrl(`https://wa.me/${number}`); }).catch(() => undefined); }, []);
  return <div className="page-shell"><SiteHeader /><main className="quote-main"><div className="quote-card">{step <= 3 && <Stepper current={step} />}
    ...
    {step === 5 && quote && <ContractSuccess form={form} contract={contract} quote={quote} whatsappUrl={whatsappUrl} />}
